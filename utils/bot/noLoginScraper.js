...
const startNoLoginScrape = async (fbPage, postCount, proxy, pUsername, pPassword) => {
    ...
    if(!await scrapeArticles(page, fbPage, Number(postCount))) await browser.close()
    
    await browser.close();
};

// startScrape(args[2],args[3],args[4],args[5],args[6])
...
const checkForBlocking = async (page) =>{
    ...
    await page.waitForSelector('div[role="dialog"][aria-label="Not Logged In"]')
    ...
    const blockingLoginPopup = await page.$('div[role="dialog"][aria-label="Not Logged In"]')
...
module.exports = {startNoLoginScrape}